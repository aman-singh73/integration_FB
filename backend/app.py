...
@app.route('/')
def home():
      return "welcome to the solar site"
...
# save image i just provided there 
...
#         new_polygon = Polygon(user_id=data['user_id'], coordinates=data['coordinates'])
#         db.session.add(new_polygon)
#         db.session.commit()
#         return jsonify({"message": "Polygon saved successfully"}), 201
#     except Exception as e:
#         db.session.rollback()
#         return jsonify({"error": str(e)}), 500
...
def save_polygon():
    try:
        ...
        # Here you could also save the coordinates to a database if needed
        # For simplicity, we're only saving the image
